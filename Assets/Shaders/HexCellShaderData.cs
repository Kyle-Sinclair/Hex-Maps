...
public class HexCellShaderData : MonoBehaviour {
    ...
    public void RefreshTerrain(HexCell cell) {
        ...
        //Debug.Log("Altering terrain cell");

        enabled = true;
    }
    public void RefreshVisiblity(HexCell cell) {
    ...
    bool UpdateCellData(HexCell cell, int delta) {
        ...
        if (cell.IsExplored && data.g < 255) {
            stillUpdating = true;
            //To progress the transition, add the delta to the cell's G value. 
            //Arithmatic operations don't work on bytes, they are always converted 
            //to integers first. So the sum is an integer, which has to be cast to a byte.
            int t = data.g + delta;
